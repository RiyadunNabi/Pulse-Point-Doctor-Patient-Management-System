...
const HealthDataSelector = ({ 
  ...
  const handleHealthLogToggle = (checked) => {
    if (disabled) return;
    setShareHealthLog(checked);
  };
    ...
    <div className="space-y-4">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <label className="flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={shareHealthLog}
                onChange={(e) => handleHealthLogToggle(e.target.checked)}
                disabled={disabled}
                className="sr-only"
              />
              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
              ...
              <span className={`ml-2 text-sm ${disabled ? 'text-slate-400' : 'text-slate-600'}`}>
                Share with doctor
              </span>
            </label>
          </div>
      ...
      {medicalDocuments.length > 0 && (
        <div className={`rounded-xl p-4 border transition-all ${
          ...
          <div className="space-y-2 max-h-32 overflow-y-auto">
            {medicalDocuments.map((doc) => (
              <div key={doc.document_id} className="flex items-center justify-between p-2 bg-slate-50/80 rounded-lg backdrop-blur-sm">
                <div className="flex-1">
                  <div className={`font-medium text-sm ${disabled ? 'text-slate-400' : 'text-slate-800'}`}>
                    {doc.file_name}
                  </div>
                  <div className={`text-xs ${disabled ? 'text-slate-400' : 'text-slate-500'}`}>
                    {doc.description && `${doc.description} • `}
                    {formatDate(doc.upload_date)}
                  </div>
                </div>
