import React, { useState } from 'react';
import { User, Edit, AlertTriangle } from 'lucide-react';
import EditProfileModal from './EditProfileModal/EditProfileModal';

const calculateAge = (dateOfBirth) => {
  const today = new Date();
  const birthDate = new Date(dateOfBirth);
  let age = today.getFullYear() - birthDate.getFullYear();
  const monthDiff = today.getMonth() - birthDate.getMonth();
  if (monthDiff < 0 || (monthDiff === 0 && today.getDate() < birthDate.getDate())) {
    age--;
  }
  return age;
};

// Helper function to check if profile is incomplete
const isProfileIncomplete = (patient) => {
  if (!patient) return true;
  
  const requiredFields = [
    'last_name',
    'gender', 
    'date_of_birth',
    'phone_no',
    'blood_group',
    'address'
  ];
  
  return requiredFields.some(field => !patient[field] || patient[field] === '');
};

function ProfileCard({ patient, onPatientUpdate }) {
  const [showEditModal, setShowEditModal] = useState(false);

  // Called by EditProfileModal after successful update
  const handleProfileUpdate = (updatedPatient) => {
    onPatientUpdate(updatedPatient);
  };

  const profileIncomplete = isProfileIncomplete(patient);

  return (
    <>
      <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/50 p-8">
        {/* Profile Completion Warning */}
        {profileIncomplete && (
          <div className="mb-6 p-4 bg-amber-50/90 backdrop-blur-sm border border-amber-200 rounded-lg">
            <div className="flex items-center space-x-3">
              <AlertTriangle className="w-5 h-5 text-amber-600" />
              <div>
                <p className="text-amber-800 font-medium">Profile Incomplete</p>
                <p className="text-amber-700 text-sm">
                  Please complete your profile information to get the best healthcare experience.
                </p>
              </div>
            </div>
          </div>
        )}

        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-6">
            {/* Profile Picture */}
            <div className="w-20 h-20 bg-gradient-to-r from-sky-400 to-cyan-400 rounded-full flex items-center justify-center">
              <User className="w-10 h-10 text-white" />
            </div>
            {/* Basic Info */}
            <div>
              <h2 className="text-3xl font-bold text-slate-800">
                {patient ? `${patient.first_name} ${patient.last_name || ''}`.trim() : 'Loading...'}
              </h2>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4 text-sm">
                <div>
                  <span className="text-slate-500">Age:</span>
                  <span className="ml-2 font-medium text-slate-700">
                    {patient?.date_of_birth ? calculateAge(patient.date_of_birth) : 'N/A'}
                  </span>
                </div>
                <div>
                  <span className="text-slate-500">Gender:</span>
                  <span className="ml-2 font-medium text-slate-700 capitalize">{patient?.gender || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-slate-500">Blood Group:</span>
                  <span className="ml-2 font-medium text-slate-700">{patient?.blood_group || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-slate-500">Phone:</span>
                  <span className="ml-2 font-medium text-slate-700">{patient?.phone_no || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-slate-500">Date of Birth:</span>
                  <span className="ml-2 font-medium text-slate-700">
                    {patient?.date_of_birth ? new Date(patient.date_of_birth).toLocaleDateString() : 'N/A'}
                  </span>
                </div>
                <div className="md:col-span-2 lg:col-span-3">
                  <span className="text-slate-500">Address:</span>
                  <span className="ml-2 font-medium text-slate-700">
                    {patient?.address || 'N/A'}
                  </span>
                </div>
              </div>
            </div>
          </div>
          {/* Edit Profile Button */}
          <button
            onClick={() => setShowEditModal(true)}
            className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-sky-600 to-cyan-600 hover:from-sky-700 hover:to-cyan-700 text-white rounded-lg shadow-md transition-all duration-200 transform hover:scale-105"
          >
            <Edit className="w-4 h-4" />
            <span>Edit Profile</span>
          </button>
        </div>

        {/* Additional Health Information (if available) */}
        {patient?.health_condition && (
          <div className="mt-6 pt-6 border-t border-slate-200">
            <h3 className="text-lg font-semibold text-slate-700 mb-2">Health Information</h3>
            <p className="text-slate-600 text-sm bg-blue-50/50 p-3 rounded-lg">
              {patient.health_condition}
            </p>
          </div>
        )}
      </div>

      {/* Edit Profile Modal */}
      <EditProfileModal
        isOpen={showEditModal}
        onClose={() => setShowEditModal(false)}
        patient={patient}
        onUpdate={handleProfileUpdate}
      />
    </>
  );
}

export default ProfileCard;




// import React, { useState } from 'react';
// import { User, Edit } from 'lucide-react';
// import EditProfileModal from './EditProfileModal/EditProfileModal';

// const calculateAge = (dateOfBirth) => {
//   const today = new Date();
//   const birthDate = new Date(dateOfBirth);
//   let age = today.getFullYear() - birthDate.getFullYear();
//   const monthDiff = today.getMonth() - birthDate.getMonth();
//   if (monthDiff < 0 || (monthDiff === 0 && today.getDate() < birthDate.getDate())) {
//     age--;
//   }
//   return age;
// };

// function ProfileCard({ patient, onPatientUpdate }) {
//   const [showEditModal, setShowEditModal] = useState(false);

//   // Called by EditProfileModal after successful update
//   const handleProfileUpdate = (updatedPatient) => {
//     onPatientUpdate(updatedPatient);
//   };

//   return (
//     <>
//       <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/50 p-8">
//         <div className="flex items-center justify-between mb-6">
//           <div className="flex items-center space-x-6">
//             {/* Profile Picture */}
//             <div className="w-20 h-20 bg-gradient-to-r from-sky-400 to-cyan-400 rounded-full flex items-center justify-center">
//               <User className="w-10 h-10 text-white" />
//             </div>
//             {/* Basic Info */}
//             <div>
//               <h2 className="text-3xl font-bold text-slate-800">
//                 {patient ? `${patient.first_name} ${patient.last_name}` : 'Loading...'}
//               </h2>
//               <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 text-sm">
//                 <div>
//                   <span className="text-slate-500">Age:</span>
//                   <span className="ml-2 font-medium text-slate-700">
//                     {patient?.date_of_birth ? calculateAge(patient.date_of_birth) : 'N/A'}
//                   </span>
//                 </div>
//                 <div>
//                   <span className="text-slate-500">Gender:</span>
//                   <span className="ml-2 font-medium text-slate-700">{patient?.gender || 'N/A'}</span>
//                 </div>
//                 <div>
//                   <span className="text-slate-500">Blood Group:</span>
//                   <span className="ml-2 font-medium text-slate-700">{patient?.blood_group || 'N/A'}</span>
//                 </div>
//                 <div>
//                   <span className="text-slate-500">Phone:</span>
//                   <span className="ml-2 font-medium text-slate-700">{patient?.phone_no || 'N/A'}</span>
//                 </div>
//               </div>
//             </div>
//           </div>
//           {/* Edit Profile Button */}
//           <button
//             onClick={() => setShowEditModal(true)}
//             className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-sky-600 to-cyan-600 hover:from-sky-700 hover:to-cyan-700 text-white rounded-lg shadow-md transition-all duration-200 transform hover:scale-105"
//           >
//             <Edit className="w-4 h-4" />
//             <span>Edit Profile</span>
//           </button>
//         </div>
//       </div>

//       {/* Edit Profile Modal */}
//       <EditProfileModal
//         isOpen={showEditModal}
//         onClose={() => setShowEditModal(false)}
//         patient={patient}
//         onUpdate={handleProfileUpdate}
//       />
//     </>
//   );
// }

// export default ProfileCard;
