...
const DoctorCard = ({ doctor, onBookAppointment }) => {
  ...
  // Only functionality change - handle click to pass doctor name
  const handleBookClick = () => {
    const doctorName = `${first_name} ${last_name}`;
    onBookAppointment(doctor_id, doctorName);
  };
    ...
    <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-white/50 hover:shadow-xl transition-all duration-300 overflow-hidden">
      ...
      <div className="p-6 pb-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${gender === 'male'
